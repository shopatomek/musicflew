...
const SongCard = ({ data, index, type }) => {
  ...
  const deleteData = (data) => {
    const deletedThisData = () => {
      dispatch({ type: actionType.SET_ALERT_TYPE, alertType: "deleted" });
      setInterval(() => {
  ...
  return (
    ...
    >
      ...
      <p className="text-base text-center text-white my-1">
        ...
        {data.artist && (
          <span className="block text-sm text-gray-300 my-2 py-0">
      ...
      {isDelete && (
        <motion.div className="absolute inset-0 backdrop-blur-md flex items-center flex-col justify-center px-4 py-2 rounded-md">
          ...
          <div className="flex items-center gap-4 py-4">
            <motion.button
              whileTap={{ scale: 0.75 }}
              className="outline-none border-none text-sm px-6 py-1 rounded-md bg-green-400 text-black hover:bg-green-500"
              onClick={(e) => {
                e.stopPropagation();
                deleteData(data);
              }}
            ...
            <motion.button
              whileTap={{ scale: 0.75 }}
              className="outline-none border-none text-sm px-6 py-1 rounded-md bg-red-400 text-black hover:animate-pulse"
              onClick={(e) => {
                e.stopPropagation();
                setisDelete(false);
              }}
