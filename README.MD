...
**Auth flow:** User signs in with Google → Firebase issues JWT token → React sends token as `Authorization: Bearer` header → Express verifies token via Firebase Admin SDK → user data stored/updated in MongoDB.
```
